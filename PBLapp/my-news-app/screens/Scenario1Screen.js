...
const Scenario1Screen = ({ navigation }) => {
  ...
  const saveData = () => {
    ...
    const newEntry = {
      policy_id: 1,
      spending_adjustment: value1.toLowerCase(),
      amount: parseFloat(text),
      timeframe: value2.toLowerCase().replace("-", "").replace(" ", ""),
    };
  ...
  return (
    ...
    >
      <ScrollView
        ...
        ref={scrollViewRef}
      >
        <Text style={styles.title}>Spending Adjustment</Text>
        <DropDownPicker
          open={open1}
          value={value1}
          items={items1}
        ...
        />
        <Text style={styles.resultText}>Selected Change: {value1}</Text>

        <Text style={[styles.title, { marginTop: 16 }]}>Percentage Change</Text>
        <TextInput
          style={styles.input}
        ...
        <DropDownPicker
          ...
          style={{ backgroundColor: "#fafafa" }}
        />
        <Text style={styles.resultText}>Selected Category: {value2}</Text>
...
export default Scenario1Screen;
